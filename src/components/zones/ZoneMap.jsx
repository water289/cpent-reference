      ...
      {zones.map(zone => {
        ...
        return (
          <Link
            key={zone.id}
            to={`/zones/${zone.id}`}
            className="group block bg-gray-800/50 border border-gray-700 rounded-lg p-6 hover:border-gray-600 transition-all duration-200"
            style={{ borderLeftWidth: '4px', borderLeftColor: color }}
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-gray-800" style={{ color }}>
